...
exports.updateDocument = async (req, res) => {
    try {
        const updatedDocument = await Entete.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updatedDocument) {
            return res.status(404).json({ message: "Document non trouvé" });
        }
...
exports.downloadDocumentPDF = async (req, res) => {
    try {
        const document = await Entete.findById(req.params.id).populate('client').populate('lignes');
        if (!document) {
            return res.status(404).json({ message: "Document non trouvé" });
        }
        ...
        doc.moveDown();
        doc.fontSize(12).text(`Date: ${new Date(document.date).toLocaleDateString()}`);
        ...
        document.lignes.forEach((ligne, index) => {
            doc.moveDown();
            doc.fontSize(12).text(`${index + 1}. ${ligne.libelleArticle} - Qté: ${ligne.quantite} - Prix HT: ${ligne.prixHT.toFixed(2)} €`);
        });
