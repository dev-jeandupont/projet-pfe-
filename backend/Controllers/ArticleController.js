...
const getArticles = async (req, res) => {
    try {
        const articles = await Article.find().populate('libelleFamille').populate('libeleCategorie');
...
const createArticle = async (req, res) => {
    ...
    try {
        ...
        if (!familleArticle || !categorieArticle) {
            return res.status(404).json({ message: "Référence non trouvée (Famille ou Catégorie)" });
        }
...
const updateArticle = async (req, res) => {
    ...
    try {
        ...
        if (!familleArticle || !categorieArticle) {
            return res.status(404).json({ message: 'Famille ou Categorie non trouvée' });
        }
